...
current_date=$(TZ=GMT LANG=C date +%s)
end_date=""
days_left=""
...
html_mode(){
	...
	while read site;do
		...
		certificate_last_day= timeout 2 $(echo | gtimeout 2 openssl s_client -servername ${sitename} -connect ${site} 2>/dev/null | \
		openssl x509 -noout -enddate 2>/dev/null | cut -d "=" -f2)
		end_date=$(TZ=GMT date -j -f '%b %d %T %Y %Z' "$certificate_last_day" "+%s")
...
terminal_mode(){
	...
	while read site;do
		sitename=$(echo $site | cut -d ":" -f1)
		certificate_last_day=$(echo | gtimeout 2 openssl s_client -servername ${sitename} -connect ${site} 2>/dev/null | \
		openssl x509 -noout -enddate 2>/dev/null | cut -d "=" -f2)
		end_date=$(TZ=GMT date -j -f '%b %d %T %Y %Z' "$certificate_last_day" "+%s")
		days_left=$(((end_date - current_date) / 86400))
